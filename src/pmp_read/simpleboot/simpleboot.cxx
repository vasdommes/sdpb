...
El::BigFloat abs(const El::BigFloat &a)
{
  return El::Abs(a);
}

template <typename T> string sign_str(T const &x)
{
  return x < 0 ? "-" : "+";
}

template <typename T> string inner_coefficient(T const &x)
{
...
template <typename T> string formula_format(std::vector<T> const &a)
{
  string result;
  if(a.size() == 0)
      ...
      if(i > 0)
        {
          ...
          if(a[i])
            result
              += " " + sign_str(a[i]) + " " + lexical_cast<string>(abs(a[i]));
        }
    }
  return result;
...
void test_load()
{
  ...
  std::ifstream ifs("saveblock_test2/A-L0.block");
  ...
  El::Output("CBTab[0][0]=", formula_format(zzb_derivs_conv_El[0][0]));

  El::BigFloat El_f = zzb_derivs_conv_El[0][0][0];
  El::Output("test El_f = ", El_f, " ; prec = ", El_f.Precision(),
             "; data_size=", El_f.SerializedSize());

  El::Output("_mp_size = ", El_f.gmp_float.get_mpf_t()[0]._mp_size,
             " ; sizeof(mp_exp_t) = ", sizeof(mp_exp_t),
             "; sizeof(mp_limb_t)=", sizeof(mp_limb_t),
             " ; sizeof(int)=", sizeof(int),
             " ; El::gmp::num_limbs=", El::gmp::num_limbs);

  std::cout.precision(200);
  std::cout << std::fixed;
  El::Output("test El_f = ", El_f);
           ...
           std::vector<std::vector<std::vector<El::BigFloat>>>> &blockF)
{
  ...
  if(pblock != blockF.end())
    MMA_PARSER_ERROR("load_block_folder error : ", stamp, "-L", spin, ".block",
                     " already exist.");
