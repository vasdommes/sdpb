...
void Abstract_Simpleboot_Data_Provider::F0(const El::BigFloat &x,
...
void Abstract_Simpleboot_Data_Provider::FS(const std::string &stamp,
                                                     ...
                                                     MMA_ELEMENT &result)
{
  result = FS(stamp, L, m, n, x);
}

void Abstract_Simpleboot_Data_Provider::PT(
  const std::string &stamp, const int L, const int m, const int n,
  const El::BigFloat &a, const El::BigFloat &b, MMA_ELEMENT &result)
{
  result = PT(stamp, L, m, n, a, b);
}
...
void Abstract_Simpleboot_Data_Provider::P(const std::string &stamp,
                                                    ...
                                                    MMA_ELEMENT &result)
{
  result = P(stamp, L, m, n, shift);
}

// (-1)^(m + n)*2^(1 + m + n - 2*x)*Poch[1 - m + x, m]*Poch[1 - n + x, n]
El::BigFloat
Abstract_Simpleboot_Data_Provider::F0(const El::BigFloat &x,
                                                const int m, const int n)
{
  ...
  return to_BigFloat(BF_result);
}

El::BigFloat
Abstract_Simpleboot_Data_Provider::F(const std::string &stamp,
                                               const int L, const int m,
                                               const int n,
                                               const El::BigFloat &x)
{
  const Polynomial poly = blockF_lookup(stamp, L, m, n);
  return poly(x) * Fprefactor(L, x);
}

El::BigFloat
Abstract_Simpleboot_Data_Provider::FS(const std::string &stamp,
                                                const int L, const int m,
                                                const int n,
                                                const El::BigFloat &x)
{
  const Polynomial poly = blockF_lookup(stamp, L, m, n);
  return poly(x) * FSprefactor(L, x);
}

Polynomial Abstract_Simpleboot_Data_Provider::PT(
  const std::string &stamp, const int L, const int m, const int n,
  const El::BigFloat &a, const El::BigFloat &b)
{
  ...
  const auto max_polynomial_degree = poly.degree() + (maxderivs - m - n);
  interval_transformation(poly.coefficients, a, b, max_polynomial_degree);

  return poly;
}

Polynomial
Abstract_Simpleboot_Data_Provider::P(const std::string &stamp,
                                               const int L, const int m,
                                               const int n,
                                               const El::BigFloat &shift)
{
...
El::BigFloat Abstract_Simpleboot_Data_Provider::Fprefactor(
  const int L, const El::BigFloat &x) const
{
  const auto &order = kappa;
  El::BigFloat denominator = 1;
  const El::BigFloat Delta = x + (L + dim - 2);
  ...
  const El::BigFloat numerator_BigFloat(
    to_BigFloat(pow(r_crossing_4, to_Boost_Float(Delta))));

  return numerator_BigFloat / denominator;
}

El::BigFloat Abstract_Simpleboot_Data_Provider::FSprefactor(
  const int L, const El::BigFloat &x) const
{
  const auto &order = kappa;
  El::BigFloat denominator = 1;
...
void Abstract_Simpleboot_Data_Provider::init_binomial_coeff(
  const int N)
{
...
mpz_class
Abstract_Simpleboot_Data_Provider::binomial_coeff_cached(const int m,
                                                                   const int n)
{
...
{
  const int N = coeff.size() - 1; // degree of the polynomial
  const int M
    = max_degree; // maximum degree of the polynomials in current matrix
...
Abstract_Simpleboot_Data_Provider::
  Abstract_Simpleboot_Data_Provider(
    ...
    : dim(params.dim),
      nu(params.nu),
      kappa(params.kappa),
      maxderivs(params.maxderivs),
      var_map(params.var_map),
      r_crossing_4(params.r_crossing_4)
{}
