...
#define MMA_PARSER_ERROR(...)                                                 \
  ...
  constexpr std::string MMA_expr_delimiters = "()[]{}+-*/^, \t\n\v\f\r";

  const char *skip_space_from_left(const char *begin, const char *end)
  {
  ...
  {
    return std::find_first_of(begin, end, delimiters.begin(),
                              delimiters.end());
  }
  ...
  parse_get_token(const char *begin, const char *end, MMA_TOKEN &token)
  {
    ptr_MMA_current = begin;
    const char *p
      = skip_space_from_left(begin, end); //skip_space_l(begin,end);
    if(p == end)
      {
        token = std::monostate{};
        return p;
    ...
    if(*p == '\\' && *(p + 1) == '\n')
      {
        RUNTIME_ERROR("the code shouldn't reach here: ",
                      std::string(p, p + 20));
      }
                                 ...
                                 MMA_TOKEN &token, char op)
  {
    ...
    if(token.index() != MMA_TOKEN_Operator
      ...
      MMA_PARSER_ERROR("parse_MMA_check_op error : expect ", op,
                       ", but I got ", token, " from text ",
                       std::string(begin, 20));
    return pstr;
  }
  ...
  parse_MMA_element(const char *begin, const char *end, MMA_ELEMENT &result)
  {
    ...
    switch(token.index())
      ...
      case MMA_TOKEN_Symbol:
        if(*pstr == '[')
          {
            pstr = parse_MMA_function(AS_MMA_TOKEN(token, Symbol), pstr + 1,
                                      end, result);
          }
        else
          {
    ...
    while(pstr != end)
      {
        ...
        MMA_PARSER_ERROR(
          "parse_MMA_expr_Times error : illegal element : ", elmt, " after ",
          chain.back(), " before '", std::string(pstr, 10), "'");
      }
  ...
  void parse_MMA_expr_subtract(MMA_EXPR &e1, MMA_EXPR &e2) const
  {
    ...
    MMA_PARSER_ERROR(
      "parse_MMA_expr_substract unexpected error : e1.index() = ", e1.index(),
      " e2.index() = ", e2.index());
  }
  void parse_MMA_expr_multiply(MMA_EXPR &e1, MMA_EXPR &e2) const
  {
    ...
    MMA_PARSER_ERROR(
      "parse_MMA_expr_multiply unexpected error : e1.index() = ", e1.index(),
      " e2.index() = ", e2.index());
  }
                                     ...
                                     std::list<MMA_ELEMENT>::iterator it) const
  {
    ...
    switch(AS_MMA_ELEMENT(*it, Operator))
      ...
      default:
        MMA_PARSER_ERROR(
          "parse_MMA_precedence unexpected error : illegal op : ",
