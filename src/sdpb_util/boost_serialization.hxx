...
{
  // Workaround for compiling with Boost 1.68.
  // Not required in 1.75 or higher.
  template <class Archive>
  void serialize(Archive &ar, uint8_t &t,
                 ...
                 const boost::serialization::version_type &version)
  {
    ...
    // NB: version=0 is used by simpleboot when loading blocks generated by scalar_blocks.
    // We cannot change it without changing scalar_blocks.
    // Optimized version=1 is used for generating and reading SDP.
    if(version == 0)
      {
        auto size = f.SerializedSize();
        std::vector<uint8_t> vec(size);
        if(Archive::is_saving::value)
          f.Serialize(vec.data());

        ar & vec;

        if(Archive::is_loading::value)
          f.Deserialize(vec.data());
        return;
      }
